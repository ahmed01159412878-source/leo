...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Admin/Developer Bypass constant
  const ADMIN_ID = "1234560";
  const ADMIN_EMAIL = "ahmed01159412878@gmail.com";
  ...
  function isAdmin() {
      const me = getMe();
      return me && me.is_master === true;
  }

  // AI BOT TRIGGER LOGIC
  function triggerBot(type, name = "") {
      const botBubble = document.getElementById('ai-bot-bubble');
      const botText   = document.getElementById('ai-bot-text');
      if (!botBubble || !botText) return;

      const firstName = name.split(' ')[0] || "يا بطل";
      
      const messagesAr = {
          welcome: `أهلاً بك يا ${firstName}! مستعد للتفوق اليوم؟ 🚀`,
          lecture: `رائع! قراءة ممتعة للمحاضرة، ركز كويس 📝`,
          video: `مشاهدة ممتعة! الفيديو ده هيفيدك جداً في الفهم 🎥`,
          exam: `حان وقت الحصاد! ركز في الامتحان الشهري، أنت بطل 🏆`,
          quiz: `بالتوفيق في الكويز! ركز وكلنا ثقة فيك 💪`,
          logout: `وداعاً يا ${firstName}، ننتظر عودتك قريباً! 👋`
      };

      const messagesEn = {
          welcome: `Welcome back, ${firstName}! Ready to excel today? 🚀`,
          lecture: `Great! Enjoy the lecture, stay focused 📝`,
          video: `Enjoy the video! This will boost your understanding 🎥`,
          exam: `Show your skills! Focus on the monthly exam, you got this 🏆`,
          quiz: `Good luck with the quiz! We believe in you 💪`,
          logout: `Goodbye, ${firstName}, see you soon! 👋`
      };

      const msg = currentLang === 'ar' ? messagesAr[type] : messagesEn[type];
      botText.textContent = msg;
      
      botBubble.classList.add('active');
      setTimeout(() => botBubble.classList.remove('active'), 5000);
  }
  ...
  function updateAuthUI() {
    ...
    if (authToken) {
      ...
      // Fetch profile info to make it feel like an "Account"
      ...
      .then(data => {
          if (data.full_name) {
              ...
              // [BOT] Trigger Welcome once per load
              if (!window.botWelcomed) {
                  triggerBot('welcome', data.full_name);
                  window.botWelcomed = true;
              }
          }
      })
      .catch(() => {
      ...
      document.querySelectorAll('.grade-card').forEach(card => {
          if (isDeveloper) {
              card.classList.remove('locked-card'); // Developer sees ALL
          } else if (card.getAttribute('data-grade') !== userGrade) {
  ...
  authBtn.addEventListener('click', () => {
    if (authToken) {
      ...
      }, 1500); // Delay for bot message
    }
  });

  // =========================================================================
  // SERVER-SIDE ACTIVATION & SECURITY
  // =========================================================================
  if (activationForm) {
    activationForm.addEventListener('submit', (e) => {
      ...
      if (fullName.split(' ').filter(w => w.length > 0).length < 3) {
          showToast(currentLang === 'ar' ? "يرجى كتابة الاسم ثلاثي بشكل صحيح" : "Please enter your full triple name", "error");
          return;
      }
      ...
      .then(result => {
          if (result.status === 200) {
              ...
              localStorage.setItem('userGrade', result.data.grade.toString());
              
              updateAuthUI();
              showToast(currentLang === 'ar' ? "تم التفعيل بنجاح! 🎉" : "Activated successfully! 🎉", "success");
              triggerBot('welcome', result.data.full_name || fullName); // [BOT] Welcome on first activation
      ...
      .catch(err => {
          showToast(currentLang === 'ar' ? "تعذر الاتصال بالخادم" : "Server connection failed", "error");
          console.error(err);
      });
    });
  }
  ...
  document.querySelectorAll('.resource-link').forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      if (materialsModal) {
        ...
        // Gating Logic Rendering
        renderMaterialsGating(grade, resourceType);
  ...
  // =========================================================================
  function renderMaterialsGating(grade, type) {
    const isDev = isAdmin();
    const items = Array.from(materialsGrid.querySelectorAll(`.material-item[data-grade="${grade}"][data-type="${type}"]`));
    const completed = JSON.parse(localStorage.getItem(`progress_${grade}_${type}`) || "[]");

    items.forEach((item, index) => {
      item.classList.remove('locked-item');
      
      // If NOT dev and NOT first item and previous NOT completed -> LOCK it
      if (!isDev && index > 0 && !completed.includes(index - 1)) {
        item.classList.add('locked-item');
      }

      // Add one-time click handler for unlocking logic
      item.onclick = (e) => {
        if (item.classList.contains('locked-item')) {
          e.preventDefault();
          showToast(currentLang === 'ar' ? "أكمل الملف السابق أولاً لقفل هذا المحتوى 🔒" : "Finish previous content to unlock 🔒", "error");
          return false;
        }

        // [BOT] Activity Triggers
        if (type === 'pdf') triggerBot('lecture');
        if (type === 'course') triggerBot('video');
        if (type === 'exam') triggerBot('exam');
        if (type === 'Quiz') triggerBot('quiz');
        
        // Mark as completed
        if (!completed.includes(index)) {
          completed.push(index);
          localStorage.setItem(`progress_${grade}_${type}`, JSON.stringify(completed));
          // Instant update for visual feedback
          setTimeout(() => renderMaterialsGating(grade, type), 500);
        }
        
        // Handle actual opening logic (open link if href exists, etc.)
        const originalLink = item.getAttribute('href');
        if (originalLink && originalLink !== "#") {
           window.open(originalLink, '_blank');
        } else {
           showToast(currentLang === 'ar' ? "جاري فتح الملف..." : "Opening file...", "success");
        }
      };
    });
  }
